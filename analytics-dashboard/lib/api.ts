...
import { sellerStatsSchema,paymentsStatsSchema,buyerStatsSchema } from "./schemas";
...
async function safeGet<T>(
  ...
  if (!url) {
    ...
    return { data: fallback, status: { online: false, errorReason: "url_not_configured" } };
  }
  ...
  try {
    const res = await fetchWithTimeout(url, { headers, next: { revalidate: 60 } });
    const latencyMs = Date.now() - t0;

    if (!res.ok) {
      const detail = `HTTP ${res.status} ${res.statusText}`;
      console.warn(`[analytics] ${label}: ${detail}`);
      return { data: fallback, status: { online: false, errorReason: "http_error", errorDetail: detail } };
    }
    ...
    try {
      ...
      console.warn(`[analytics] ${label}: ${detail}`);
      return { data: fallback, status: { online: false, errorReason: "parse_error", errorDetail: detail, latencyMs } };
    }
    ...
    const detail = isTimeout ? `Timeout después de ${FETCH_TIMEOUT_MS}ms` : `Error de red: ${err}`;
    console.warn(`[analytics] ${label}: ${detail}`);
    return { data: fallback, status: { online: false, errorReason, errorDetail: detail, latencyMs } };
  }
}
...
export async function fetchSellerStats(): Promise<{ data: SellerStats; status: AppStatus }> {
  return safeGet<SellerStats>(
    SELLER_APP_URL ? `${SELLER_APP_URL}/api/analytics` : undefined,
    SELLER_APP_API_KEY,
    mockSellerStats,
    "Seller App",
    sellerStatsSchema
